...
llava_lm_state_dict = llava_lm.model.layers.state_dict()
desta_lm_state_dict = desta_lm.model.layers.state_dict()
llama_state_dict = llama.model.layers.state_dict()

# List of merging configuration
merge_config_list = [
    {
        'merge_method': 'linear',
        'alpha': 1.0,
...
save_path = [
    os.path.join(save_dir, f"{x.get('merge_method')}-{x.get('alpha')}.ckpt")
    for x in merge_config_list
]
for idx, merge_config in enumerate(tqdm(merge_config_list)): 
    ...
    torch.save(merged_state_dict, save_path[idx])
